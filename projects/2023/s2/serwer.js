...
app.engine(
  ...
  hbs({
    ...
    helpers: {
      dolar: function (price) {
        let prased = new Number(price).toPrecision(3);
        if (`${prased}`.split(".")[1].length < 2) {
          prased += "0";
        }
        return `${prased} $`;
      },
  ...
  items: [
    {
      ...
      stars: 6,
      category: "Apparel",
      img_url: "http://4ia1.spec.pl.hostingasp.pl/test_uploadu/star.png",
      price: 29.99,
    },
    {
      title: "Coffee Mug",
      ...
      reviews: [
        {
          ...
          stars: 2,
          date: 1.455800194995e12,
        },
        {
          name: "Bob",
          comment: "Love this.",
          stars: 5,
          date: 1.455804800769e12,
        },
        {
          name: "Jorge",
          comment: "Brown. It's brown.",
          stars: 4,
          date: 1.455804825509e12,
        },
      ],
    },
...

    {
      ...
      reviews: [
        ...
        },
        {
          name: "Shannon",
          comment:
            "Sturdy construction, but a little too big to fit in my bag for work.",
          stars: 3,
...
function parseData(data) {
  return data.map((el) => {
    return {
      ...el,
      starList: [...Array(parseInt(el.stars))].map((_) => el.img_url),
    };
  });
}

function filterData(data, stars) {
  return data.filter((el) => el.stars == stars);
}

function generateStarList(data) {
  const stars = [];
  for (const el of data) {
    stars.push(el.stars);
  }
  const setStar = new Set(stars);
  return [...setStar].sort();
}
...
function generateCategoryList(data) {
  ...
  const setStar = new Set(categorys);
  return [...setStar].sort();
}
...
app.get("/", (req, res) => {
  const radioList = generateStarList(data.items);
  const selectList = generateCategoryList(data.items);
  res.render("table.hbs", {
    data: parseData(data.items),
    radioList: radioList,
    selectList: selectList,
  });
});

app.post("/handleFliter", (req, res) => {
  const { stars } = req.body;
  res.render("result.hbs", {
    data: filterData(parseData(data.items), stars),
  });
});
// static
